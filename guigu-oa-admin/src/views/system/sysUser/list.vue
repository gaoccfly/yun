  ...
  methods: {
    ...
    getRoles () {
      roleApi.getRoles(this.sysUser.id).then(response => {
        const {allRoleList, assginRoleList} = response.data
        this.allRoles = allRoleList
        this.userRoleIds = assginRoleList.map(item => item.id)
        this.checkAll = allRoleList.length===assginRoleList.length
        this.isIndeterminate = assginRoleList.length>0 && assginRoleList.length<allRoleList.length
      })
    },
